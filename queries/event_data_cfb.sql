SELECT 
    ...
    g.home_conf_name AS "home_conf_name",
    g.home_division_type AS "home_division_type",
    G.away_team AS "away_team_id",
    g.away_team_name || ' ' || g.away_team_nickname AS "away_team",
    g.away_team_abbrev AS "away_team_abbrev",
    g.away_conf_name AS "away_conf_name",
    g.away_division_type AS "away_division_type",
    g.home_score AS "home_final_score",
    g.away_score AS "away_final_score",
    g.home_score - g.away_score AS "final_score_diff",
    CASE when G.quarters > 4 THEN 0
        ELSE g.home_score - g.away_score END AS "end_of_regulation_score_diff",
    CASE WHEN E.off_team_id = G.home_team
        THEN G.home_score - E.off_start_score ELSE G.home_score - E.def_start_score END AS "home_rest_of_game_score",
    CASE WHEN E.off_team_id = G.away_team
        THEN G.away_score - E.off_start_score ELSE G.away_score - E.def_start_score END AS "away_rest_of_game_score",
    CASE when G.quarters > 4 THEN (CASE WHEN E.off_team_id = G.home_team THEN 1 ELSE -1 END) * (E.off_start_score - E.def_start_score)
        ELSE (g.home_score - g.away_score) - (CASE WHEN E.off_team_id = G.home_team THEN 1 ELSE -1 END) * (E.off_start_score - E.def_start_score) END AS "end_of_regulation_score_diff_change",
    CASE WHEN E.off_team_id = G.home_team
    ...
    CASE WHEN g.home_score > g.away_score THEN 'W'
        ...
        WHEN g.home_score = g.away_score THEN 'T' END as "home_team_outcome",
    CASE WHEN g.home_score > g.away_score THEN 1
        WHEN g.home_score < g.away_score THEN 0
        WHEN g.home_score = g.away_score THEN 0 END as "home_team_win",
    CASE WHEN g.home_score > g.away_score THEN 0
        WHEN g.home_score < g.away_score THEN 0
        WHEN g.home_score = g.away_score THEN 1 END as "draw",
    CASE WHEN g.home_score > g.away_score THEN 0
        WHEN g.home_score < g.away_score THEN 1
        WHEN g.home_score = g.away_score THEN 0 END as "away_team_win",
    E.nevent AS "nevent",
...
FROM customer_data.cd_football_events E
    LEFT JOIN customer_data.cd_cfb_games G
